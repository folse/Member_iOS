...
class WIndicator:UIView {
   ...
   class func showIndicatorAddedTo(view:UIView, animation:Bool) -> WActivityIndicator {
    ...
    class func showSuccessInView(view: UIView, text:String, timeOut interval:NSTimeInterval) -> WIndicatorImage {
        
        var indicatorImageView = WIndicatorImage(view: view, imageName: "success", text: text, timeOut: interval)
