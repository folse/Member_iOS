...
class WIndicatorImage: UIView {
    ...
    convenience init(view:UIView, imageName:String, text:String, timeOut interval:NSTimeInterval) {
        ...
        let imageViewWidth:Float = Float(imageView.frame.size.width)
        let imageViewHeight:Float = Float(imageView.frame.size.height)
        
        var bgViewWidth = imageViewWidth + 2 * margin
        var bgViewHeight = imageViewHeight + 2 * margin
        var bgViewX:Float = (Float(view.bounds.size.width) - ( bgViewWidth )) / 2
        var bgViewY:Float = (Float(view.bounds.size.height) - (bgViewHeight )) / 2
        
        var labelX:Float = (bgViewWidth - imageViewWidth)/2
        var labelY:Float = (bgViewHeight - imageViewWidth)/2
        
        bgView!.frame = CGRect(x: CGFloat(bgViewX), y: CGFloat(bgViewY), width: CGFloat(bgViewWidth), height: CGFloat(bgViewHeight))
        imageView.frame = CGRect(x: CGFloat(labelX), y: CGFloat(16), width: CGFloat(imageViewWidth), height: CGFloat(imageViewHeight))
